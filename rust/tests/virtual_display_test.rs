...
fn test_get_virtual_display_status() {
    ...
    assert!(!status.modes.is_empty(), "Should provide default virtual display modes");
    assert!(status.modes.iter().any(|m| m.width == 1920 && m.height == 1080), "Should include 1080p mode");
    assert!(status.modes.iter().any(|m| m.refresh_rate == 120), "Should include 120Hz mode");
...
fn test_generate_headless_frame_valid_dimensions() {
    ...
    assert_eq!(frame.len(), (width * height * 4) as usize, "Buffer must match width * height * 4");
...
fn test_driver_inf_file_validity() {
    ...
    assert!(path_to_check.exists(), "IddSampleDriver.inf must exist in repository");
    let content = std::fs::read_to_string(path_to_check).expect("Failed to read INF file");
    assert!(content.contains("[Version]"), "INF must have [Version] section");
    assert!(content.contains("Class=Display"), "INF must specify Class=Display");
    assert!(content.contains("Root\\VrVDeskIdd"), "INF must define Hardware ID");
}
