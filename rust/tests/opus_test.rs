...
fn test_opus_encode_decode_roundtrip() {
    ...
    let mut encoder = Encoder::new(sample_rate, channels, Application::Audio).expect("create encoder");
    encoder.set_bitrate(opus::Bitrate::Bits(64_000)).expect("set bitrate");
    ...
    println!("PCM size: {} bytes, Opus encoded size: {} bytes (compression ratio: {:.1}x)",
             pcm_in.len() * 2, encoded_len, (pcm_in.len() * 2) as f32 / encoded_len as f32);

    assert!(encoded_len > 0);
    assert!(encoded_len < 300, "Opus 20ms packet should be compact (< 300 bytes)");

    // Decode back
    let mut pcm_out = vec![0i16; frame_size * 2];
    let decoded_samples = decoder.decode(&opus_out[..encoded_len], &mut pcm_out, false).expect("decode opus");
